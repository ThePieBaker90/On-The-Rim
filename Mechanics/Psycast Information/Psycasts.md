...
- Some psycasts may require concentration during the psycasts duration. Concentration may only be held on 1 psycast at a time. Concentration can be dispelled by the psycaster at anytime for free, which immediately moves the psycast to the end phase. 
	- While concentrating on psycasts, your baseline [[Neural Heat]] may not drop below a specified amount. This amount is indicated in parenthesis next to the initial [[Neural Heat]] cost.
...
- Affliction effects are effects that affect only afflicted creatures. Psycasts only afflict creatures if explicitly stated, additionally psycasts mentioning afflicted creatures only refer to creatures afflicted by the same psycast instance.
	...
	- If an affliction grants a [[Conditions|Condition]], unless the [[Conditions|Condition]] has a specified duration, it is removed when the creature loses the affliction.
### Delayed Effects
...
- Psycasts may be used stealthily to avoid detection or observation. They may be stealthily cast while the caster is [[Stealth and Hiding#Undetected|Undetected]] or [[Stealth and Hiding#Unnoticed|Unnoticed]] by the target. 
- The drawbacks of stealth casting depend on psycast's components (the specific effects are listed in [[#Psycast Components]]). 
	- If a psycast has multiple components, you make a check for each component.
- When using a psycast while [[Stealth and Hiding#Undetected|Undetected]] or [[Stealth and Hiding#Unnoticed|Unnoticed]], all other rules of stealth must be obeyed.
